document.addEventListener("DOMContentLoaded", function () {
  ...
  function obtenerHoraArgentina() {
    const horaUTC = new Date().toLocaleTimeString("en-US", {
  ...
  function agregarComentarioALaLista(nombre, email, mensaje, hora) {
    ...
    comentarioElemento.innerHTML = `<strong>Comentario:</strong> Nombre: ${nuevoComentario.nombre}, Email: ${nuevoComentario.email}, Mensaje: ${nuevoComentario.mensaje}, Hora: ${nuevoComentario.hora},Calificación: 
  ...
  if (comentarios.length > 0) {
    ...
    agregarComentarioALaLista(
      ultimoComentario.nombre,
      ultimoComentario.email,
      ultimoComentario.mensaje,
      ultimoComentario.hora
    );
  }
